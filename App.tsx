...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = (currentTime: number) => {
      if (!isPlaying) {
         previousTime = currentTime; // Keep previousTime fresh while paused
      ...
      // Time scale: 
      // At speed 1, we want reasonable movement. 
      // Let's say 1 "Earth Year" (period=1) takes about 60 seconds at 1x speed.
      // 1 year = 1.0 unit. 
      // 60 seconds = 60,000 ms.
      // Increment per ms = 1.0 / 60000 = 0.0000166.
      // Default formula: time += deltaTime * BASE_RATE * speed
  ...
  return (
    <div className="relative w-full h-screen bg-slate-950 flex overflow-hidden">
      ...
      {/* Sidebar (Overlay on mobile, panel on desktop if space allows) */}
      {selectedBody && (
        <InfoPanel 
            body={selectedBody} 
